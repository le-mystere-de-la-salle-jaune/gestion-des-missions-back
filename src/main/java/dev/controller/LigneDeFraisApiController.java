...
public class LigneDeFraisApiController extends BaseApiController<LigneDeFrais, NoteDeFrais, LigneDeFraisVM> {
	...
	// } else {
	// return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Cette ligne de
	// frais n'existe pas");
	// }
	// }
	//
	...
	// */
	// @PostMapping("/{id}")
	// public ResponseEntity<String> update(@RequestBody LigneDeFraisVM
	// ligneDeFraisVM, @PathVariable Long id) {
	// if (this.ligneDeFraisService.exist(id)) {
	// LigneDeFrais ligneDeFrais =
	// ligneDeFraisService.findById(ligneDeFraisVM.getId());
	...
	//
	// return ResponseEntity.status(HttpStatus.OK)
	// .body("La ligne de frais dont l'id est " + ligneDeFraisVM.getId() + " a
	// été modifiée.");
	// } else {
	// return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Aucune ligne de
